...
function outp = SamplerSchachtlerTECO(Teile, anzTeile, zulRot, Margin, auflosungsF, gewFlache, gewKoll, gewSt, Bild, Farbbild, Ident)
...
parfor i=1:2   % 就是paralle + for的意思, 因为并行处理的，所以32个task的顺序不是一致的
    try
    ...
    for piece=1:numOfPieces
        index = randi(length(Teile));   % 获取(0, Teile数量)之间的随机数
        Teile_Rand{piece,1} = Teile{index,1};   % 随机获取一个Teile，扔到Teile_rank中
        %Rotation_Rand(piece) = Rotation(index)
        %Rotation_Rand(piece) = zulRot(randperm(length(zulRot)));
        
        Rotation_Rand(piece) =  zulRot(randi(length(zulRot)));   % 随机获取一个Rotation，并扔到Rotation_rand中
    end
    %Rotation_Rand = str2num(Rotation_Rand);
    fprintf('After random selectio of Teile %s\n', datestr(now, 'HH:MM:SS.FFF'));
    [B, B2, BLeng] = getPartsTECO(Teile_Rand, auflosungsF, zulRot_Rand); % Liste der Teile ？？？
    fprintf('After placing the Teile in the box %s\n', datestr(now, 'HH:MM:SS.FFF'));
    Id = int2str(i); 

    Pop = horzcat(1:1:BLeng, 0, Rotation_Rand);   % horzcat应该是horizon catenate
    [Pop(1, BLeng+1), D{1}, E{1}]= FitnessBLSN(A, B, B2, C, Pop(1, :), zulRot_Rand, auflosungsF, Margin, BLeng, gewFlache, gewKoll, gewSt, Id);
    Pop(1, BLeng+1);
    fprintf('After calculating the Fitness %s\n', datestr(now, 'HH:MM:SS.FFF'));
