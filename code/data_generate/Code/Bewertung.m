function [matxOut, localSearch, StutzPX] = Bewertung(C, B2, am, stutz, i, Margin, PopI, rotind, auflosungsF)
...
elseif size(unique(StutzPX),1)==2 || size(unique(StutzPX),1)==1 % Auflagepunkte in einer Reihe: Teil kippt
    ...
    if inpolygon(matxOut(3),matxOut(4),StutzPX,StutzPY) && inpolygon(matxOut(1),matxOut(2),StutzPX, StutzPY)
        ...
        if in==0 || on==1
            ...
            % nächsten Auflagepunkt finden, Achsen
...
% % Loggen
% fileID6 = fopen('HebelLogErstePlatzierung.txt','a');
% fprintf(fileID6,'%7.3f ', i);
% fprintf(fileID6,'%7.3f ', Fm);
% fprintf(fileID6,'%7.3f ', Fp);
% fprintf(fileID6,'%7.3f ', dm1);
% fprintf(fileID6,'%7.3f ', dp1);
% fprintf(fileID6,'%7.3f ', dm2);
% fprintf(fileID6,'%7.3f ',dp2);
% fprintf(fileID6,'%7.3f ',Mk1);
% fprintf(fileID6,'%7.3f ',Mk2);
% fprintf(fileID6,'%7.3f ',matxOut(1));
% fprintf(fileID6,'%7.3f ',matxOut(2));
% fprintf(fileID6,'%7.3f ',matxOut(3));
% fprintf(fileID6,'%7.3f ',matxOut(4));
% fprintf(fileID6,'%s ',g1);
% fprintf(fileID6,'%s ',g2);
% fprintf(fileID6,'%s \r\n',g3);
% 
% for i =1:length(StutzPX)
%     fprintf(fileID6,'%7.3f ', StutzPX(i));
%     fprintf(fileID6,'%7.3f \r\n', StutzPY(i));
% end
% fclose(fileID6);
